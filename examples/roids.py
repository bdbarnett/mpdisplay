...
def main():
    ...
    class Poly:
        ...
        def __init__(
            ...
            self.polygon = (
                polygon
                if scale is None
                else [(int(scale * x[0]), int(scale * x[1])) for x in polygon]
            )
            ...
            self.velocity_x = (
                random.uniform(0.50, 0.99) * 6 - 3 + 0.75 if v_x is None else v_x
            )
            self.velocity_y = (
                random.uniform(0.50, 0.99) * 6 - 3 + 0.75 if v_y is None else v_y
            )
    ...
    while True:
        ...
        if ship_alive:
            ...
            if (
                buttons.fire
                and buttons.fire.value() == 0
                and len(missiles) < missile_max
            ):
