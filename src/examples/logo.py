...
def main():
    ...
    graphics.round_rect(display_drv, LEFT_X, TOP_Y, int(BASE_UNIT*1.2), int(BASE_UNIT*1.0), BASE_UNIT//7, pal.BLACK, True)
    ...
    graphics.fill_rect(display_drv, display_drv.width - (LEFT_X + SIZE), display_drv.height - (TOP_Y + SIZE), SIZE, SIZE, pal.BLACK)

    # Draw the 2 small circles
    SIZE = SIZE // 2
    graphics.circle(display_drv, display_drv.width - (LEFT_X + SIZE), TOP_Y + SIZE, SIZE, pal.BLACK, True)
    graphics.circle(display_drv, LEFT_X + SIZE, display_drv.height - (TOP_Y + SIZE), SIZE, pal.BLACK, True)
