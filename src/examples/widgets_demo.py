...
color_wheel = pd.get_palette(name="wheel", swapped=display.needs_swap, length=display.vsa, saturation=1.0)
if screen.partitioned:
...
aligns = [pd.ALIGN.TOP_LEFT, pd.ALIGN.TOP, pd.ALIGN.TOP_RIGHT,
          pd.ALIGN.LEFT, pd.ALIGN.CENTER, pd.ALIGN.RIGHT,
          pd.ALIGN.BOTTOM_LEFT, pd.ALIGN.BOTTOM, pd.ALIGN.BOTTOM_RIGHT,
          pd.ALIGN.OUTER_TOP_LEFT, pd.ALIGN.OUTER_TOP, pd.ALIGN.OUTER_TOP_RIGHT,
          pd.ALIGN.OUTER_LEFT, pd.ALIGN.OUTER_RIGHT,
          pd.ALIGN.OUTER_BOTTOM_LEFT, pd.ALIGN.OUTER_BOTTOM, pd.ALIGN.OUTER_BOTTOM_RIGHT]
...
down = pd.IconButton(top, align=pd.ALIGN.TOP_RIGHT, icon_file=pd.icon_theme.down_arrow(pd.ICON_SIZE.LARGE))
up = pd.IconButton(top, align_to=down, align=pd.ALIGN.OUTER_LEFT, icon_file=pd.icon_theme.up_arrow(pd.ICON_SIZE.LARGE))
slider1 = pd.Slider(top, y=9, w=up.x-toggle.x-toggle.width-16, h=18, align=pd.ALIGN.TOP, value=0, step=0.05)
...
button = pd.Button(main, w=main.width//2, h=64, align=pd.ALIGN.CENTER, label="test", radius=10, pressed_offset=5)
...
button.add_event_cb(pd.Events.MOUSEBUTTONDOWN, lambda sender, e: status.set_value("Button pressed"))
