...
def add_item(to_list: pd.ListView):
    global item_no
    item = pd.Button(to_list, label=f"Item {item_no}", icon_file=pd.icon_theme.home(pd.ICON_SIZE.LARGE))
    item.add_event_cb(pd.Events.MOUSEBUTTONDOWN, lambda sender, e: status.set_value(f"{sender.label.value} clicked."))
    item_no += 1
