...
def draw_logo(canvas):
    ...
    pygraphics.circle(canvas, center_x, center_y, unit, blue, True)
    pygraphics.circle(canvas, center_x, center_y, int(unit * .9), black, True)
    pygraphics.circle(canvas, center_x, center_y, int(unit * .8), amber, True)

    # Draw the outer rounded rectangle
    left = int(center_x - (unit * 1.2) // 2)
    top = int(center_y - (unit * 1.0 ) // 2)
    pygraphics.round_rect(canvas, left, top, int(unit*1.2), int(unit*1.0), unit//7, black, True)

    # Draw the inner rounded rectangle
    left = int(center_x - (unit * 1.1) // 2)
    top = int(center_y - (unit * 0.9) // 2)
    pygraphics.round_rect(canvas, left, top, int(unit*1.1), int(unit*0.9), unit//9, amber, True)
    ...
    pygraphics.fill_rect(canvas, left, top, size, size, black)
    pygraphics.fill_rect(canvas, w - (left + size), h - (top + size), size, size, black)

    # Draw the 2 small circles
    size = size // 2
    pygraphics.circle(canvas, w - (left + size), top + size, size, black, True)
    pygraphics.circle(canvas, left + size, h - (top + size), size, black, True)
