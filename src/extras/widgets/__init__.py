...
class Screen(Widget):
    ...
    def draw(self):
        ...
        """
        self.display.draw_obj.fill_rect(*self.area, self.bg)


class Button(Widget):
    def __init__(self, parent: Widget, x=None, y=None, w=_default_icon_size, h=_default_icon_size, fg=_white, visible=True, value=None,
    ...
    def draw(self):
        ...
        if self._pressed:
            self.display.draw_obj.fill_rect(*draw_area, self.bg)
            draw_area = Area(
        ...
        self.display.draw_obj.round_rect(*draw_area, self.radius, self.fg, f=self.filled)
...
class Label(Widget):
    ...
    def draw(self):
        ...
        if self.bg is not None:
            self.display.draw_obj.fill_rect(*self.abs_area, self.bg)  # Draw background if bg is specified
        x, y, _, _ = self.abs_area
        self.display.draw_obj.text(self.value, x, y, self.fg, height=self.height)
...
class TextBox(Widget):
    ...
    def draw(self):
        ...
        Draw the label's text on the screen, using absolute coordinates.
        """
        self.display.draw_obj.fill_rect(*self.abs_area, self.bg)
        self.display.draw_obj.text(self.value, self.x+self.margin, self.y+self.margin, self.fg, height=self.text_height)


class ProgressBar(Widget):
    def __init__(self, parent: Widget, x=None, y=None, w=64, h=_default_icon_size, fg=_black, bg=_white,
    ...
    def draw(self):
        ...
        if self.vertical:
            ...
            if self.reverse:
                ...
                self.display.draw_obj.fill_rect(self.x, self.y, self.width, progress_height, self.fg)
            else:
                # Fill from bottom to top (default)
                self.display.draw_obj.fill_rect(self.x, self.y + self.height - progress_height, self.width, progress_height, self.fg)
        else:
            # Calculate the width of the filled part
            progress_width = int(self.value * self.width)
            if self.reverse:
                # Fill from right to left
                self.display.draw_obj.fill_rect(self.x + self.width - progress_width, self.y, progress_width, self.height, self.fg)
            else:
                # Fill from left to right (default)
                self.display.draw_obj.fill_rect(self.x, self.y, progress_width, self.height, self.fg)
        ...
        for y in range(0, h):
            for x in range(0, w):
                if (c := pixels[(y * w + x) * planes + alpha]) != 0:
                    self.display.draw_obj.pixel(pos_x + x, pos_y + y, pal[c])


class IconButton(Button):
    def __init__(self, parent: Widget, x=None, y=None, w=_default_icon_size, h=_default_icon_size, fg=_black, bg=_white,
...
class CheckBox(IconButton):
    def __init__(self, parent, x=None, y=None, w=_default_icon_size, h=_default_icon_size, fg=_black, bg=_white,
...
class RadioButton(IconButton):
    def __init__(self, parent, group: RadioGroup, x=None, y=None, w=_default_icon_size, h=_default_icon_size, fg=_black, bg=_white,
                 visible=True, value=False):
        """
...
class Slider(ProgressBar):
    def __init__(self, parent, x=None, y=None, w=100, h=_default_icon_size, fg=_black, bg=_white, visible=True,
    ...
    def draw(self):
        ...
        self.display.draw_obj.circle(*knob_center, self.knob_radius, self.knob_color, f=True)
...
def main():
    ...
    display_drv.fill_rect(0, 0, 10, 1, -1)
    ...
    pbtn1 = w.Button(screen, x=pbar.x-_default_icon_size, y=pbar.y, fg=pal.GREEN)
