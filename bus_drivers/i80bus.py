...
class I80Bus(_BaseBus):
    ...
    def __init__(self, dc, cs, wr, data0, data1, data2, data3, data4, data5, data6, data7, cs_active_high=False, dc_data_level=1, pclk_active_neg=False, swap_color_bytes=False) -> None:
        ...
        self._mask_wr_inactive, self._mask_wr_active = self.gpio.get_set_clr_masks(wr, pclk_active_neg)
        ...
        self._wr_inactive_reg, self._wr_active_reg = self.gpio.get_set_clr_regs(wr, pclk_active_neg)
            ...
            if value != self._last:  # No need to set the data pins again if the value hasn't changed
                ...
                for i, lut in enumerate(self._lookup_tables):
                    ...
                    if self._pin_masks[i]:
                        ...
                        self._tx_value = lut[value]  # 32-bit value
                        if self._use_set_clr_regs:  # 32-bit ports
                            # Set the appropriate bits for all 32 pins
                            machine.mem32[self._set_regs[i]] = self._tx_value
                            # Clear the appropriate bits for all 32 pins
                            machine.mem32[self._clr_regs[i]] = self._tx_value ^ self._pin_masks[i]
                        else:  # 16-bit ports
                            # Set and clear the bits for the first 16 pins
                            machine.mem32[self._set_reset_regs[i][0]] = (self._tx_value & 0xFFFF) | ((self._tx_value ^ self._pin_masks[i]) << 16)
                            # Set and clear the bits for the second 16 pins
                            machine.mem32[self._set_reset_regs[i][1]] = (self._tx_value >> 16) | ((self._tx_value ^ self._pin_masks[i]) & 0xFFFF0000)
                self._last = value
