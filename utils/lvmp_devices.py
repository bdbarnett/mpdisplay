...
devices = Devices(display_drv=display_drv, bgr=False, factor=4,
                  blit_func=display_drv.blit, alloc_buf_func=display_drv.allocate_buffer,
...
####################### Generic methods
    ...
    def _indev_setup(self, indev):
        ...
        indev.set_disp(self.display if self.display else lv.disp_get_default())
...
#        print(f"Blitting ({area.x1}, {area.y1}) to ({area.x2}, {area.y2})")
    ...
    def _get_buf(self, width, height, factor, double_buffer, alloc_buf_func):
        buf_size = (width * height * lv.color_t.__SIZE__) // factor
        self._buf1 = alloc_buf_func(buf_size) if alloc_buf_func else bytearray(buf_size)
        self._buf2 = alloc_buf_func(buf_size) if alloc_buf_func else bytearray(buf_size) if double_buffer else None
...
####################### Touch methods
    ...
    def set_touch_rotation(self, touch_rotation):
        ...
        self._max_x = self.display.get_hor_res() - 1
        self._max_y = self.display.get_ver_res() - 1
